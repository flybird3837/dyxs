                    ...
                    <div class="list-group bg-white site-list">
                        @foreach($map->sites as $site)
                            <div class="list-group-item" data-id="{{$site->id}}">
                                <span>{{$site->name}}</span>
                                <a href="javascript:void(0);" class="pull-right complete-draw hidden" data-id="{{$site->id}}">完成</a>
                                @if($site->points)
                                    <a href="javascript:void(0);" class="pull-right re-draw" data-id="{{$site->id}}">重绘</a>
                                @else
                                    <a href="javascript:void(0);" class="pull-right draw" data-id="{{$site->id}}">添加</a>
                                @endif
                            </div>
                        @endforeach
                    </div>
...
@section('scripts')
    <script>
        ...
        const mapUlr = '{{$mapUrl}}';
        new Site({container: ".canvas-container", map: mapUlr});
